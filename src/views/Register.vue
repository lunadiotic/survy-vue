      ...
      <form class="mt-8 space-y-6" action="#" @submit.prevent="register">
        <div class="rounded-md shadow-sm -space-y-px">
          ...
          <div>
            ...
            <input id="password" name="password_confirmation" type="password" autocomplete="current-password" required="" class="appearance-none rounded-none relative block w-full px-3 py-2 border border-gray-300 placeholder-gray-500 text-gray-900 rounded-b-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 focus:z-10 sm:text-sm" placeholder="Retype Password" v-model="user.password_confirmation" />
          </div>
        </div>
        <div>
        ...
        <div class="text-center">
          <div class="text-sm">
            <router-link :to="{ name: 'Login' }" class="font-medium text-indigo-600 hover:text-indigo-500"> Already have an account? Login Here </router-link>
          </div>
        </div>
      </form>
    </div>
